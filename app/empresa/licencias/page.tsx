...
export default function LicensesPage() {
  ...
  return <EnterpriseShell title="Licencias" subtitle="Capacidad y consumo por módulo. Cada usuario consume una licencia en cada módulo asignado.">
    {!data ? <p>Cargando...</p> : <div className="space-y-7">
      ...
      <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">{data.licenses.map((l)=>{const usage=Math.round(l.used_seats/l.seat_capacity*100); return <SectionCard key={l.module_key} title={l.module_key === "sales" ? "Sales Hub" : l.module_key === "expense" ? "Expense Hub" : l.module_name} eyebrow="Módulo independiente" action={<StatusPill>{l.license_status === "trial" ? `${l.trial_days_remaining} días de prueba` : l.license_status}</StatusPill>}><div className="grid grid-cols-3 gap-3"><Mini label="Usadas" value={l.used_seats}/><Mini label="Libres" value={l.available_seats}/><Mini label="Total" value={l.seat_capacity}/></div><div className="mt-6 h-2 overflow-hidden rounded-full bg-[#E8ECEF]"><div className="h-full bg-[#00B8AE]" style={{width:`${Math.min(usage,100)}%`}}/></div><div className="mt-4 flex items-center justify-between text-sm"><span className="text-[#69717D]">Uso: {usage}%</span><strong>{l.license_status === "trial" ? `Vence en ${l.trial_days_remaining} días` : `US$ ${l.monthly_total_usd}/mes`}</strong></div><button className="mt-6 w-full rounded-full bg-[#0B0C0E] px-5 py-3 text-sm font-black text-white">Comprar bloque de 10</button></SectionCard>})}</div>
