...
import { profilesFor, profileLabel } from "../../../src/lib/moduleProfiles";

type ModuleAssignment = { license_id:string; module_key:string; module_name:string; role:string; status:string };
type ApiUser = { id:string; role:string; status:string; profile_id:string; wama_profiles:{ id:string; full_name:string; email:string; status:string } | null; invitation:{status:string;sent_at?:string|null;send_attempts?:number;last_error?:string|null}|null; module_assignments:ModuleAssignment[] };
...
export default function UsersPage() {
  ...
  async function updateTenantRole(profileId:string,tenantRole:"admin"|"member") { const key=`tenant:${profileId}`; setSavingKey(key); setMessage(""); const accessToken=await token(); const response=await fetch("/api/enterprise/users",{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${accessToken}`},body:JSON.stringify({profileId,tenantRole})}); const data=await response.json(); setSavingKey(""); if(!response.ok){setMessage(`Error: ${data.error||"No se pudo actualizar el nivel empresarial."}`);return;} setMessage(tenantRole==="admin"?"Administrador empresarial habilitado con acceso total a los módulos activos.":"Nivel empresarial actualizado."); await load(); }

  async function resend(profileId:string){setSavingKey(`invite:${profileId}`);setMessage("");const accessToken=await token();const response=await fetch("/api/enterprise/users",{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:`Bearer ${accessToken}`},body:JSON.stringify({profileId})});const data=await response.json();setSavingKey("");if(!response.ok){setMessage(`Error: ${data.error||"No se pudo reenviar la invitación."}`);return;}setMessage(`Invitación confirmada por el proveedor para ${data.email}.`);await load();}

  const canAdmin=["owner","admin","super_admin"].includes(role.toLowerCase());
  const selectedModules=Object.keys(form.moduleRoles);
  const activeModuleNames=licenses.map((license)=>license.wama_module_catalog.name);
  const accessCopy=activeModuleNames.length===1
    ? `Esta empresa tiene ${activeModuleNames[0]} activo. Cada persona asignada consume una licencia y puede tener un perfil propio dentro del módulo.`
    : activeModuleNames.length>1
      ? `Tu empresa tiene ${activeModuleNames.join(", ")} activos. Cada módulo consume una licencia y una misma persona puede tener perfiles diferentes en cada uno.`
      : "Activa un módulo para comenzar a asignar licencias y perfiles a tu equipo.";
  return <EnterpriseShell title="Usuarios" subtitle="Administra tu equipo y asigna un perfil diferente en cada módulo.">
    <div className="space-y-7">
      <section className="rounded-[2rem] bg-gradient-to-br from-white to-[#F0FBFA] p-6 shadow-[0_18px_50px_rgba(25,45,55,.07)]"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#008F87]">Equipo y licencias</p><h2 className="mt-2 text-2xl font-black">Personas con acceso a tu empresa</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#69717D]">{accessCopy}</p></div><button disabled={!canAdmin} onClick={()=>setOpen(true)} className="cursor-pointer rounded-full bg-[#00E5D6] px-6 py-3.5 text-sm font-black text-black shadow-[0_10px_24px_rgba(0,229,214,.25)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-40">+ Invitar usuario</button></div></section>
      ...
      <SectionCard title="Equipo de la empresa" eyebrow="Roles independientes por módulo" action={null}>
        <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead><tr className="border-b border-[#E8ECEF] text-xs uppercase text-[#69717D]"><th className="pb-4">Usuario</th><th className="pb-4">Estado</th>{licenses.map((license)=><th key={license.id} className="pb-4">{license.wama_module_catalog.name}</th>)}</tr></thead><tbody>{users.map((user)=><tr key={user.id} className="border-b border-[#EEF1F3] last:border-0"><td className="py-5"><strong className="block">{user.wama_profiles?.full_name||"Usuario"}</strong><span className="text-[#69717D]">{user.wama_profiles?.email}</span>{user.role==="owner"?<span className="mt-1 block text-xs font-black text-[#008F87]">Propietario</span>:user.role==="admin"?<span className="mt-1 block text-xs font-black text-[#008F87]">Administrador empresarial</span>:canAdmin?<select aria-label="Nivel empresarial" disabled={savingKey===`tenant:${user.profile_id}`} value={user.role==="admin"?"admin":"member"} onChange={(event)=>void updateTenantRole(user.profile_id,event.target.value as "admin"|"member")} className="mt-2 rounded-lg border border-[#DDE8E7] bg-white px-2 py-1 text-xs font-bold"><option value="member">Miembro de empresa</option><option value="admin">Administrador empresarial</option></select>:null}</td><td className="py-4 pr-5">{user.status!=="invited"?<StatusPill>Activo</StatusPill>:<div className="min-w-[180px]"><StatusPill>{user.invitation?.status==="failed"?"Envío fallido":user.invitation?.status==="sent"?"Correo enviado":"Pendiente"}</StatusPill>{user.invitation?.sent_at&&<span className="mt-1 block text-xs text-[#69717D]">{new Date(user.invitation.sent_at).toLocaleString("es-CL")}</span>}{user.invitation?.last_error&&<span className="mt-1 block text-xs text-red-600">{user.invitation.last_error}</span>}{canAdmin&&<button onClick={()=>void resend(user.profile_id)} disabled={savingKey===`invite:${user.profile_id}`} className="mt-2 text-xs font-black text-[#008F87] underline disabled:opacity-50">{savingKey===`invite:${user.profile_id}`?"Reenviando…":"Reenviar invitación"}</button>}</div>}</td>{licenses.map((license)=>{const moduleKey=license.wama_module_catalog.module_key;const assignment=user.module_assignments.find((item)=>item.module_key===moduleKey);const key=`${user.profile_id}:${moduleKey}`;return <td key={license.id} className="py-4 pr-4">{assignment?(user.role==="owner"?<StatusPill>Acceso total</StatusPill>:<select disabled={!canAdmin||savingKey===key} value={assignment.role} title={profileLabel(moduleKey,assignment.role)} onChange={(event)=>void updateModuleRole(user.profile_id,moduleKey,event.target.value)} className="w-full min-w-[220px] cursor-pointer rounded-xl bg-[#F5F8F9] px-3 py-2.5 font-bold outline-none ring-[#00BEB3] focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60">{optionsFor(moduleKey).map((option)=><option key={option.value} value={option.value}>{option.label}</option>)}</select>):<span className="text-[#9AA2AA]">Sin acceso</span>}</td>;})}</tr>)}</tbody></table>{!users.length&&!message&&<p className="py-8 text-center text-[#69717D]">Cargando usuarios…</p>}</div>
