...
export async function POST(request: Request) {
  try {
    ...
    const { data: tenant, error: tenantError } = await admin.from("wama_tenants").insert({
      name: companyName, slug, status: "trial", trial_ends_at: trialEnds, pilot_name: "Piloto Expense 15 días",
    ...
    if (!expenseModule) return NextResponse.json({ error: "No existe el módulo Expense en el catálogo." }, { status: 500 });
    ...
    if (licenseError || !license) return NextResponse.json({ error: licenseError?.message || "No se pudo activar Expense." }, { status: 400 });
    ...
    return NextResponse.json({ ok: true, tenant, ownerEmail, seats: 10, module: "expense" });
  } catch (error) {
