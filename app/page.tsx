...
const modules = [
  ...
  },
  {
    ...
    price: "US$10", href: "/modulos/operations-hub", trial: "operations",
  },
];
...
function ModuleCard(module:(typeof modules)[number]){const Icon=module.icon;return <article className="flex h-full flex-col rounded-[2rem] border border-[#D8DFE4] bg-white p-6 shadow-[0_18px_55px_rgba(11,12,14,.07)] sm:p-7"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#00E5D6]/15 text-[#008F87]"><Icon className="h-6 w-6"/></span><p className="mt-6 text-xs font-black uppercase tracking-[.18em] text-[#008F87]">{module.eyebrow}</p><h3 className="mt-2 text-3xl font-black">{module.name}</h3><p className="mt-4 text-xl font-black leading-snug">{module.title}</p><p className="mt-4 leading-7 text-[#66707C]">{module.description}</p><ul className="mt-6 space-y-3">{module.features.map(f=><li key={f} className="flex gap-3 text-sm font-bold"><Check className="h-5 w-5 shrink-0 text-[#008F87]"/>{f}</li>)}</ul><div className="mt-auto pt-8"><p className="text-3xl font-black">{module.price}<span className="text-sm text-[#66707C]"> / módulo / mes</span></p><p className="mt-2 text-sm text-[#66707C]">Incluye hasta 10 usuarios · prueba gratis 15 días</p><div className="mt-5 flex gap-3"><Link href={`/trial?module=${module.trial}`} className="flex-1 rounded-full bg-[#00E5D6] px-5 py-3 text-center text-sm font-black">Probar gratis</Link><Link href={module.href} className="rounded-full border border-[#CBD3D9] px-5 py-3 text-center text-sm font-black">Conocer</Link></div></div></article>}
