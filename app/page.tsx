...
const modules = [
  ...
  },
  {
    ...
    price: "US$20", href: "/modulos/expense-hub", trial: "expense",
  },
  {
  ...
  return <WamaShell><main className="overflow-hidden bg-white text-[#0B0C0E]">
    <section className="relative bg-[#0B0C0E] text-white">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-6 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:py-28">
        <div><p className="text-sm font-black uppercase tracking-[.24em] text-[#00E5D6]">WAMA · Warn and Manage</p><h1 className="mt-6 max-w-5xl text-5xl font-black leading-[.94] tracking-[-.065em] sm:text-6xl md:text-7xl">Tres módulos. Un solo portal para gestionar tu empresa.</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-[#B7BEC8]">Ventas, rendiciones de gastos y operaciones trabajan en el mismo ambiente, con los mismos usuarios y una experiencia conectada.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><Link href="/trial" className="rounded-full bg-[#00E5D6] px-8 py-4 text-center text-sm font-black text-[#0B0C0E]">Probar gratis 15 días</Link><Link href="/acceso" className="rounded-full border border-white/20 px-8 py-4 text-center text-sm font-black text-white">Ingresar a mi portal</Link></div><a href="mailto:contacto@wamaapp.com" className="mt-6 inline-flex text-sm font-bold text-[#00E5D6]">¿Tienes consultas? contacto@wamaapp.com</a></div>
    ...
    <section className="bg-white"><div className="mx-auto max-w-7xl px-5 py-20 sm:px-6 lg:py-28"><div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><p className="text-sm font-black uppercase tracking-[.22em] text-[#008F87]">Un mismo ambiente</p><h2 className="mt-5 text-4xl font-black leading-tight tracking-[-.055em] md:text-6xl">Un acceso. Tres flujos conectados.</h2><p className="mt-5 text-lg leading-8 text-[#66707C]">El owner administra empresa, módulos, usuarios, licencias y facturación desde un portal. Cada persona entra únicamente a las herramientas y funciones que le corresponden.</p></div><div className="rounded-[2rem] border border-[#DDE3E7] bg-[#F7F9FA] p-6 sm:p-8"><Flow number="01" title="Crea tu empresa" text="Configura identidad y owner una sola vez."/><Flow number="02" title="Activa tus módulos" text="Inicia 15 días gratis por Sales, Expense u Operations."/><Flow number="03" title="Invita y asigna usuarios" text="Entrega acceso y rol según cada módulo."/><Flow number="04" title="Gestiona desde el mismo portal" text="Cambia de proceso sin salir del ambiente WAMA."/></div></div></div></section>
