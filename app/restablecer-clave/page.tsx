...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    async function prepareRecovery() {
      const verifiedAt = Number(window.sessionStorage.getItem("wama_recovery_verified") || 0);
      const { data, error: sessionError } = await supabase.auth.getSession();
      if (!active) return;
      if (!sessionError && data.session && Date.now() - verifiedAt < 30 * 60 * 1000) {
        setReady(true);
        return;
      }
      setError("Verifica primero el código recibido por correo para crear una nueva contraseña.");
    }
