...
export default function ResetPasswordPage() {
  ...
  return <WamaShell><section className="mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl place-items-center px-6 py-16"><WamaCard className="w-full max-w-xl p-7 sm:p-9"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#00E5D6]/15 text-[#00E5D6]"><KeyRound className="h-6 w-6"/></span><h1 className="mt-6 text-4xl font-black tracking-[-.04em] text-white">Crea una nueva contraseña.</h1>{ready?<form onSubmit={submit} className="mt-7 grid gap-4"><Password label="Nueva contraseña" value={password} show={show} onChange={setPassword} onToggle={()=>setShow(!show)}/><Password label="Repetir contraseña" value={confirm} show={show} onChange={setConfirm}/>{error&&<div className="rounded-2xl border border-red-400/30 bg-red-500/10 p-4 text-sm text-red-100">{error}</div>}<button disabled={loading} className="cursor-pointer rounded-full bg-[#00E5D6] px-5 py-4 text-sm font-black text-[#0B0C0E] disabled:opacity-50">{loading?"Guardando…":"Guardar nueva contraseña"}</button></form>:<div className="mt-6 rounded-2xl bg-white/5 p-5 text-sm leading-6 text-[#C4C7CC]">Abre esta pantalla desde el enlace recibido por correo. Si el enlace expiró, solicita una nueva recuperación.</div>}</WamaCard></section></WamaShell>;
}
