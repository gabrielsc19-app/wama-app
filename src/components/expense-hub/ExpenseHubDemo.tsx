...
function NewExpense({onSubmit}:{onSubmit:(e:Expense)=>void}) {
  ...
  const [documentMeta,setDocumentMeta]=useState({documentType:"",siiDocumentCode:null as number|null,rutValid:null as boolean|null,totalsMatch:null as boolean|null,reviewRequired:true,validationStatus:"review",duplicateKey:"",engine:""});
  ...
  const applyOcr=(data:any)=>{
    ...
    setDocumentMeta({
      ...
      siiDocumentCode:data.siiDocumentCode==null?null:Number(data.siiDocumentCode),
      rutValid:typeof data.rutValid==="boolean"?data.rutValid:null,
      totalsMatch:typeof data.totalsMatch==="boolean"?data.totalsMatch:null,
      reviewRequired:Boolean(data.reviewRequired),
      validationStatus:String(data.validationStatus||"review"),
      duplicateKey:String(data.duplicateKey||""),
      engine:String(data.engine||"WAMA Chile DTE Engine v1"),
    });
    setStep(2);
  };
  ...
  const readFile=async(ev:ChangeEvent<HTMLInputElement>)=>{
    ...
    if(!selected)return;
    setFile(selected);setFileName(selected.name);setOcrError("");setQuotaError(false);setConfidence(null);setWarnings([]);setDocumentMeta({documentType:"",siiDocumentCode:null,rutValid:null,totalsMatch:null,reviewRequired:true,validationStatus:"review",duplicateKey:"",engine:""});
...
function ChileCheck({label,value}:{label:string;value:boolean|null}) {
  const text=value===true?"Correcto":value===false?"Revisar":"No disponible";
  const cls=value===true?"text-[#217A39]":value===false?"text-[#A33030]":"text-[#7A838E]";
  return <div><p className="text-xs text-[#69717D]">{label}</p><p className={`mt-1 font-black ${cls}`}>{text}</p></div>;
}
