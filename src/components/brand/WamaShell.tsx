...
import { Download, Home, Layers3, LogIn, Menu, PlayCircle, X } from "lucide-react";
...
const mobileItems = [
  ...
  { href: "/acceso", label: "Demo", icon: PlayCircle },
    ...
    <div className="min-h-screen bg-[#0B0C0E] pb-20 text-[#F5F6F7] md:pb-0">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0B0C0E]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 md:h-auto md:py-5">
          ...
          <div className="hidden items-center gap-2 md:flex">
            <Link href="/acceso" className="rounded-full border border-white/15 bg-white/[0.04] px-5 py-3 text-sm font-black transition hover:border-[#00E5D6]/50 hover:text-[#00E5D6]">Probar demo</Link>
            <Link href="/login" className="rounded-full border border-white/15 px-5 py-3 text-sm font-black transition hover:border-[#00E5D6]/50 hover:text-[#00E5D6]">Ingresar</Link>
            <Link href="/trial" className="rounded-full bg-[#00E5D6] px-5 py-3 text-sm font-black text-[#0B0C0E] transition hover:-translate-y-0.5">Activa tu prueba gratis</Link>
          </div>

          <div className="flex items-center gap-2 md:hidden">
            <Link href="/login" className="inline-flex h-10 items-center justify-center rounded-full bg-[#00E5D6] px-4 text-xs font-black text-[#0B0C0E]">Ingresar</Link>
        ...
        {open && (
          <div className="border-t border-white/10 bg-[#0B0C0E] px-4 py-4 md:hidden">
            <div className="mx-auto grid max-w-md gap-2">
              ...
              <Link href="/descargar-app" onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-black"><Download className="h-5 w-5 text-[#00E5D6]" />Descargar app</Link>
              <Link href="/trial" onClick={() => setOpen(false)} className="mt-1 rounded-2xl bg-[#00E5D6] px-4 py-3 text-center text-sm font-black text-[#0B0C0E]">Activa tu prueba gratis</Link>
            </div>
          </div>
        )}
      </header>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-[70] border-t border-[#DCE3E8] bg-white/95 px-2 pb-[max(.45rem,env(safe-area-inset-bottom))] pt-2 text-[#0B0C0E] shadow-[0_-12px_30px_rgba(11,12,14,.12)] backdrop-blur-xl md:hidden">
        <div className="mx-auto grid max-w-md grid-cols-4">
          {mobileItems.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} className="flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-[10px] font-black"><Icon className="h-5 w-5 text-[#29323A]" /><span className="truncate">{label}</span></Link>
          ))}
        </div>
      </nav>
