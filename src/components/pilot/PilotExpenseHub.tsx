...
export default function PilotExpenseHub() {
  ...
  async function operate(action:string){
    ...
    const d=await r.json(); if(!r.ok){setError(d.error||"No se pudo registrar la acción.");setActionLoading(false);return;} setSelected(null);setEvidence([]);setPaymentAmount("");setMessage("Movimiento actualizado correctamente.");await load();setActionLoading(false);
  }
...
{canFinance&&["approved","pending_payment","partially_paid"].includes(selected.status)&&<div className="mt-6 border-t pt-5"><p className="text-xs font-black uppercase tracking-[.14em] text-[#69717D]">Pago de la solicitud</p><p className="mt-2 text-sm text-[#69717D]">Registra un abono o el pago total aprobado.</p><div className="mt-3 grid gap-3"><input type="number" min="1" value={paymentAmount} onChange={e=>setPaymentAmount(e.target.value)} placeholder="Monto del abono" className="rounded-2xl border border-[#DDE3E7] px-4 py-3 text-sm"/><button disabled={actionLoading||!Number(paymentAmount)} onClick={()=>operate("pay")} className="rounded-full bg-[#00E5D6] px-5 py-3 text-sm font-black disabled:opacity-50">{actionLoading?"Registrando…":"Registrar abono o pago"}</button></div></div>}
</section></div></div></div>}
  </EnterpriseShell>;
}
