...
type Payload={cases:Case[];projects:Project[];locations:Ref[];categories:Ref[];teams:Ref[];members:Person[];notifications:Notice[];setup?:{completed_at?:string|null}|null;profile:Person;moduleRole:string;canAdmin:boolean;canCoordinate:boolean;canWork:boolean;license:{used:number;capacity:number;blocks:number;status:string};error?:string};
...
export default function OperationsHub(){
 ...
 useEffect(()=>{
  ...
  const newCaseHandler=()=>setOpen(true);
  window.addEventListener("wama:operations:view",viewHandler);
  window.addEventListener("wama:operations:new-case",newCaseHandler);
  return()=>{
   window.removeEventListener("wama:operations:view",viewHandler);
   window.removeEventListener("wama:operations:new-case",newCaseHandler);
  };
 },[]);
 ...
 async function deleteCase(){
  ...
  if(reason.length<5){setError("Indica un motivo de eliminación de al menos 5 caracteres.");return;}
  if(!window.confirm(`¿Eliminar ${selected.case_number}? El caso quedará archivado y podrá restaurarse.`))return;
  setBusy(true);setError("");setSuccess("");
  try{
 ...
 async function assignScope(value:string){
 ...
 return <EnterpriseShell title="Operations Hub" subtitle="Reporta, asigna y controla cada incidente operacional desde un solo lugar.">
  <div className="space-y-4 pb-4 xl:space-y-6 xl:pb-0">
   {data?.projects?.length?<section className="mobile-operations-project flex flex-col justify-between gap-3 rounded-2xl border border-[#DCE1E6] bg-white p-4 lg:flex-row lg:items-center"><label className="grid flex-1 gap-2 sm:max-w-md"><span className="text-xs font-black uppercase tracking-[.14em] text-[#008F87]">Proyecto activo</span><select value={selectedProjectId} onChange={e=>{setSelectedProjectId(e.target.value);setForm({...empty,projectId:e.target.value})}} className="input"><option value="">Todos mis proyectos</option>{data.projects.map(project=><option key={project.id} value={project.id}>{project.name}</option>)}</select></label><OperationsPushStatus/></section>:null}
   <section className="hidden overflow-hidden rounded-[2rem] bg-[#0B0C0E] p-6 text-white sm:p-8 xl:block"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-center"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#00E5D6]">WAMA · Operaciones</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">Cada caso tiene responsable, plazo e historial.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#B8C0C8]">Deja atrás reportes dispersos. Centraliza incidentes, alertas urgentes y evidencias de todas tus ubicaciones.</p></div><button onClick={()=>setOpen(true)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#00E5D6] px-6 py-4 font-black text-black"><Plus className="h-5 w-5"/>Reportar caso</button></div></section>
   <nav className="hidden gap-2 overflow-x-auto rounded-2xl border border-[#DCE1E6] bg-white p-2 xl:flex">{[{id:"summary",name:"Resumen",icon:LayoutDashboard},{id:"projects",name:"Proyectos",icon:Building2},{id:"cases",name:"Casos",icon:FileText},{id:"alerts",name:"Alertas",icon:ShieldAlert},{id:"notifications",name:`Avisos${data?.notifications?.length?` (${data.notifications.length})`:""}`,icon:Bell},{id:"teams",name:"Equipos",icon:Users},{id:"users",name:"Usuarios",icon:Users},{id:"reports",name:"Informes",icon:BarChart3},{id:"settings",name:"Configuración",icon:Settings}].map(i=><button key={i.id} onClick={()=>setView(i.id)} className={`inline-flex shrink-0 items-center gap-2 rounded-xl px-4 py-3 text-sm font-black ${view===i.id?"bg-[#0B0C0E] text-white":"text-[#65707B]"}`}><i.icon className="h-4 w-4"/>{i.name}</button>)}</nav>
   ...
   {loading?<div className="grid h-64 place-items-center rounded-[2rem] bg-white"><Loader2 className="animate-spin text-[#008F87]"/></div>:<>
    ...
    {view==="notifications"&&<section className="rounded-[2rem] border border-[#DCE1E6] bg-white p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[#008F87]">Centro de avisos</p><h2 className="mt-2 text-2xl font-black">Lo que requiere tu atención</h2></div><Bell className="text-[#008F87]"/></div><div className="mt-6 divide-y">{data?.notifications.map(n=><article key={n.id} className="py-4"><strong>{n.title}</strong><p className="mt-1 text-sm text-[#56616C]">{n.body}</p><small className="mt-2 block text-[#7A838D]">{new Date(n.created_at).toLocaleString("es-CL")}</small></article>)}{!data?.notifications.length&&<p className="py-10 text-center text-sm text-[#69717D]">Estás al día. No tienes avisos pendientes.</p>}</div></section>}
    {view==="teams"&&<OperationsTeamWorkspace teams={data?.teams||[]} cases={cases} members={data?.members||[]} selectedId={selectedTeamId} select={setSelectedTeamId} openCase={openCase} edit={item=>editConfig("team",item)} invite={()=>setView("users")}/>} 
    {view==="users"&&(data?.canAdmin?<OperationsUsersPanel/>:<section className="rounded-[2rem] border bg-white p-8 text-center"><Users className="mx-auto text-[#008F87]"/><h2 className="mt-4 text-xl font-black">Administración protegida</h2><p className="mt-2 text-sm text-[#69717D]">Solo el owner o administrador puede gestionar licencias e invitaciones.</p></section>)}
    {view==="reports"&&<Reports cases={cases} teams={data?.teams||[]}/>} 
    {view==="settings"&&<SettingsView canAdmin={canAdmin} data={data!} onAdd={setConfigOpen} onEdit={editConfig}/>} 
   </>}
  </div>
...
function SetupWizard({profile,members,license,busy,error,finish}:{profile:Person;members:Person[];license:Payload["license"];busy:boolean;error:string;finish:(items:{location:string;team:string;category:string;receivesUrgent:boolean;slaMinutes:number;memberIds:string[];invites:SetupInvite[]})=>Promise<void>}){
 const[step,setStep]=useState(0),[location,setLocation]=useState(""),[team,setTeam]=useState(""),[category,setCategory]=useState(""),[receivesUrgent,setReceivesUrgent]=useState(true),[slaMinutes,setSlaMinutes]=useState(1440),[invites,setInvites]=useState<SetupInvite[]>([]),[selectedMemberIds,setSelectedMemberIds]=useState<string[]>(members.filter(item=>item.id!==profile.id).map(item=>item.id));
 const steps=["Bienvenida","Ubicación","Equipo","Integrantes","Categoría","Confirmación"];
 const normalizedEmails=invites.map(item=>item.email.trim().toLowerCase()).filter(Boolean);
 const duplicateEmail=normalizedEmails.some((email,index)=>normalizedEmails.indexOf(email)!==index);
 const invalidInvite=invites.some(item=>!item.fullName.trim()||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(item.email.trim()));
 const licensedEmails=new Set(members.map(item=>item.email.trim().toLowerCase()));
 const newEmails=new Set(normalizedEmails.filter(email=>!licensedEmails.has(email)));
 const usedAfter=license.used+newEmails.size;
 const nextDisabled=(step===1&&!location.trim())||(step===2&&!team.trim())||(step===3&&(invalidInvite||duplicateEmail||usedAfter>license.capacity))||(step===4&&!category.trim());
 const addInvite=()=>setInvites(items=>[...items,{fullName:"",email:"",role:"operations_operator"}]);
 const updateInvite=(index:number,patch:Partial<SetupInvite>)=>setInvites(items=>items.map((item,i)=>i===index?{...item,...patch}:item));
 return <div className="mx-auto max-w-5xl pb-24"><section className="overflow-hidden rounded-[2rem] border border-[#DCE1E6] bg-white shadow-sm"><div className="bg-[#0B0C0E] px-6 py-7 text-white sm:px-10"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#00E5D6] text-black"><Sparkles className="h-5 w-5"/></span><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#00E5D6]">Primera configuración</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Crea tu Operations Hub</h1></div></div><p className="mt-4 max-w-2xl text-sm leading-6 text-[#C4C7CC]">Define ubicaciones, equipos, integrantes y plazos. Cada correo se licencia una sola vez en Operations, aunque participe en varios equipos.</p></div><div className="border-b px-6 py-5 sm:px-10"><div className="mb-3 flex items-center justify-between text-xs font-black"><span>Paso {step+1} de {steps.length}</span><span className="text-[#008F87]">{steps[step]}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#E8EDEE]"><div className="h-full rounded-full bg-[#00B8AE] transition-all" style={{width:`${(step+1)/steps.length*100}%`}}/></div></div><div className="min-h-[400px] p-6 sm:p-10">
 {step===0&&<WizardIntro name={profile.full_name}/>} 
 {step===1&&<WizardText icon={<MapPin/>} title="¿Dónde pueden ocurrir los casos?" text="Crea tu primera ubicación. Puede ser una sucursal, edificio, tienda, oficina o instalación."><Field label="Nombre de la ubicación"><input autoFocus value={location} onChange={e=>setLocation(e.target.value)} className="input" placeholder="Ej. Oficina Providencia"/></Field><Suggestions values={["Casa matriz","Sucursal","Bodega","Oficina"]} choose={setLocation}/></WizardText>}
 {step===2&&<WizardText icon={<Users/>} title="¿Qué equipo atenderá los casos?" text="Define el primer equipo. Luego podrás crear otros sin volver a consumir cupos por las mismas personas."><Field label="Nombre del equipo"><input autoFocus value={team} onChange={e=>setTeam(e.target.value)} className="input" placeholder="Ej. Operaciones"/></Field><Suggestions values={["Operaciones","Mantención","Seguridad","Administración"]} choose={setTeam}/><label className="mt-5 flex items-start gap-3 rounded-2xl bg-red-50 p-4"><input type="checkbox" checked={receivesUrgent} onChange={e=>setReceivesUrgent(e.target.checked)}/><span><strong className="block text-sm">Recibir alertas urgentes</strong><span className="text-xs text-[#69717D]">Los integrantes configurados serán avisados una sola vez ante un caso crítico.</span></span></label></WizardText>}
 {step===3&&<WizardText icon={<Users/>} title="¿Quiénes formarán el equipo?" text="El owner ya ocupa el primer cupo. Selecciona usuarios licenciados o agrega correos nuevos; pertenecer a más equipos no vuelve a consumir licencia."><div className="rounded-2xl border border-[#BFE8E4] bg-[#F2FFFD] p-4"><div className="flex items-center justify-between gap-3"><div><strong className="block text-sm">{profile.full_name||"Owner"}</strong><span className="text-xs text-[#69717D]">{profile.email} · Owner y coordinador</span></div><Pill text="Licenciado"/></div></div>{members.filter(item=>item.id!==profile.id).map(member=><label key={member.id} className="flex cursor-pointer items-center gap-3 rounded-2xl border border-[#DCE1E6] p-4"><input type="checkbox" checked={selectedMemberIds.includes(member.id)} onChange={e=>setSelectedMemberIds(ids=>e.target.checked?[...ids,member.id]:ids.filter(id=>id!==member.id))}/><span className="flex-1"><strong className="block text-sm">{member.full_name}</strong><span className="text-xs text-[#69717D]">{member.email} · Ya consume un cupo en Operations</span></span></label>)}{invites.map((invite,index)=><div key={index} className="grid gap-3 rounded-2xl border border-[#DCE1E6] p-4 sm:grid-cols-2"><input value={invite.fullName} onChange={e=>updateInvite(index,{fullName:e.target.value})} className="input" placeholder="Nombre completo"/><input type="email" value={invite.email} onChange={e=>updateInvite(index,{email:e.target.value})} className="input" placeholder="correo@empresa.cl"/><select value={invite.role} onChange={e=>updateInvite(index,{role:e.target.value as SetupInvite["role"]})} className="input sm:col-span-2"><option value="operations_coordinator">Coordinador</option><option value="operations_operator">Operativo</option><option value="operations_reporter">Reportante</option><option value="operations_observer">Observador</option></select><button type="button" onClick={()=>setInvites(items=>items.filter((_,i)=>i!==index))} className="text-left text-xs font-black text-red-600 sm:col-span-2">Quitar integrante</button></div>)}<button type="button" onClick={addInvite} disabled={usedAfter>=license.capacity} className="inline-flex items-center justify-center gap-2 rounded-full border border-[#00B8AE] px-5 py-3 text-sm font-black text-[#08766F] disabled:opacity-40"><Plus className="h-4 w-4"/>Agregar integrante por correo</button><div className="rounded-2xl bg-[#F4F6F7] p-4 text-sm"><strong>{usedAfter} de {license.capacity} cupos</strong> quedarán utilizados. {Math.max(0,license.capacity-usedAfter)} permanecerán disponibles.</div>{duplicateEmail&&<p className="text-sm font-bold text-red-600">Un correo está repetido. Cada persona debe ingresarse una sola vez.</p>}{usedAfter>license.capacity&&<p className="text-sm font-bold text-red-600">No quedan cupos suficientes. Contrata un paquete adicional de 10 usuarios para continuar.</p>}</WizardText>}
 {step===4&&<WizardText icon={<Clock3/>} title="¿Qué tipo de caso controlarás primero?" text="Las categorías ordenan los reportes y establecen el plazo esperado de respuesta."><Field label="Nombre de la categoría"><input autoFocus value={category} onChange={e=>setCategory(e.target.value)} className="input" placeholder="Ej. Incidente operacional"/></Field><Suggestions values={["Incidente operacional","Falla técnica","Seguridad","Aseo"]} choose={setCategory}/><Field label="Plazo máximo de respuesta"><select value={slaMinutes} onChange={e=>setSlaMinutes(Number(e.target.value))} className="input"><option value={240}>4 horas</option><option value={480}>8 horas</option><option value={1440}>24 horas</option><option value={2880}>48 horas</option></select></Field></WizardText>}
 {step===5&&<WizardText icon={<CheckCircle2/>} title="Tu módulo está listo para ser creado" text="Revisa la configuración. Las invitaciones se enviarán al confirmar."><div className="grid gap-3 sm:grid-cols-4"><Summary icon={<MapPin/>} label="Ubicación" value={location}/><Summary icon={<Users/>} label="Equipo" value={team}/><Summary icon={<Clock3/>} label="Categoría" value={category}/><Summary icon={<Users/>} label="Licencias" value={`${usedAfter}/${license.capacity}`}/></div><div className="mt-5 rounded-2xl bg-[#F4F8F8] p-4 text-sm text-[#4F5964]">Plazo inicial: <strong>{slaMinutes/60} horas</strong> · Alertas urgentes: <strong>{receivesUrgent?"Activadas":"Desactivadas"}</strong> · Invitaciones: <strong>{invites.length}</strong></div></WizardText>}
 {error&&<div className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">{error}</div>}</div><footer className="flex items-center justify-between gap-3 border-t p-5 sm:px-10"><button onClick={()=>setStep(Math.max(0,step-1))} disabled={step===0||busy} className="inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-black text-[#65707B] disabled:opacity-30"><ArrowLeft className="h-4 w-4"/>Anterior</button>{step<5?<button onClick={()=>setStep(step+1)} disabled={nextDisabled} className="inline-flex items-center gap-2 rounded-full bg-[#00E5D6] px-6 py-3 text-sm font-black text-black disabled:opacity-40">{step===0?"Comenzar":"Continuar"}<ArrowRight className="h-4 w-4"/></button>:<button onClick={()=>finish({location,team,category,receivesUrgent,slaMinutes,memberIds:selectedMemberIds,invites})} disabled={busy||nextDisabled} className="inline-flex items-center gap-2 rounded-full bg-[#00E5D6] px-6 py-3 text-sm font-black text-black disabled:opacity-50">{busy?<Loader2 className="h-4 w-4 animate-spin"/>:<CheckCircle2 className="h-4 w-4"/>}{busy?"Creando e invitando…":"Crear mi Operations Hub"}</button>}</footer></section><p className="mt-4 text-center text-xs text-[#7A838D]">Un mismo usuario puede pertenecer a varios equipos y seguirá consumiendo un solo cupo de Operations.</p></div>;
}
