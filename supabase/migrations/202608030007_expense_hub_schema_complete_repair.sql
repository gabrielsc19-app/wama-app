...
-- Reparacion acumulativa e idempotente de Expense Hub.
-- Puede ejecutarse aunque las migraciones 005 o 006 hayan quedado parciales.
...
update public.wama_expense_reports
set requested_amount_clp = amount_clp
where requested_amount_clp is null;

alter table public.wama_expense_reports
  drop constraint if exists wama_expense_reports_status_check;

alter table public.wama_expense_reports
  add constraint wama_expense_reports_status_check check (
    status in (
      'draft','submitted','assigned','in_review','observed','approved','rejected',
      'pending_payment','partially_paid','paid','open','partially_rendered',
      'settled','closed'
    )
  );

alter table public.wama_expense_reports
  drop constraint if exists wama_expense_reports_request_type_check;

alter table public.wama_expense_reports
  add constraint wama_expense_reports_request_type_check check (
...
create table if not exists public.wama_expense_payments (
  ...
  payment_type text not null default 'payment'
    check (payment_type in ('advance','installment','reimbursement','balance_return')),
...
create index if not exists idx_wama_expense_assignee
  on public.wama_expense_reports(tenant_id, assigned_to, status);
create index if not exists idx_wama_expense_parent_fund
  on public.wama_expense_reports(parent_fund_id, status);
create index if not exists idx_wama_expense_payments_report
  on public.wama_expense_payments(report_id, paid_at desc);
create index if not exists idx_wama_expense_events_report
  on public.wama_expense_events(report_id, created_at desc);
...
create policy wama_expense_payments_member
  on public.wama_expense_payments for select to authenticated
...
create policy wama_expense_events_member
  on public.wama_expense_events for select to authenticated
  using (public.wama_is_tenant_member(tenant_id));
...
-- Fuerza a la API de Supabase a reconocer todas las columnas inmediatamente.
