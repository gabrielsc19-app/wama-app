-- P0.3 CORRECCION WAMA
-- Modelo comercial correcto:
-- US$10/mes por módulo, incluye hasta 10 usuarios.
-- Cada bloque adicional de 10 usuarios cuesta US$10/mes.
begin;
...
alter table public.wama_tenant_module_licenses
  ...
  alter column extra_block_price_usd set default 10;
...
as $$
  select
    ...
    (
      l.unit_price_usd
      + (l.extra_seat_blocks * l.extra_block_price_usd)
    )::numeric,
    l.starts_at,
    l.renews_at,
    case
  ...
  group by
    t.id, t.name, t.code,
    mc.module_key, mc.name,
    l.id, l.status,
    l.included_seats, l.extra_seat_blocks, l.extra_block_size,
    l.unit_price_usd, l.extra_block_price_usd,
    l.starts_at, l.renews_at
